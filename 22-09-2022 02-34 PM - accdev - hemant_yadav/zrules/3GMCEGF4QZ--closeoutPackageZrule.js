function formatDate(date) {
	if (date) {
		var d = new Date(new Date(date + ' UTC').toLocaleString('en-us', {
		...
		return ([year, month, day].join('-') + ' ' + time + ' (' + contextVar.gmtTimeZone + ')');
	} else {
		return '-';
	}
}
					...
					if (arrayCheck(element.serviceGridWrapper)) {
						if (contextVar.fileType == 'Closeout Package PDF') {
							...
							element.serviceGridWrapper.forEach(function (serviceElement) {
								...
								appendString = appendString.replace('COMPLETEDON', formatDate(serviceElement.addedOn));
								appendString = appendString.replace('SERVICETYPE', serviceElement.selectedService.serviceName);
								appendString = appendString.replace('SERVICEMODE', serviceElement.serviceMode);
								...
								appendDetailsString = appendDetailsString.replace('ADDEDON', formatDate(serviceElement.addedOn));
								appendDetailsString = appendDetailsString.replace('SERVICETYPE', serviceElement.selectedService.serviceName);
								...
								switch (serviceElement.serviceMode) {
									...
									//Other Section
									case 'assetOthers':
										if (checkForData(serviceElement[serviceElement.serviceMode].others.serialNumber)) {
											appendActionDetailsString = new String(contextVar.assetServicedService);
											appendActionDetailsString = appendActionDetailsString.replace('MANUFACTURER', checkForData(serviceElement[serviceElement.serviceMode].others.manufacturer) ? serviceElement[serviceElement.serviceMode].others.manufacturer : '-');
											appendActionDetailsString = appendActionDetailsString.replace('MODELNAME', checkForData(serviceElement[serviceElement.serviceMode].others.modelName) ? serviceElement[serviceElement.serviceMode].others.modelName : '-');
											appendActionDetailsString = appendActionDetailsString.replace('MODELNUMBER', checkForData(serviceElement[serviceElement.serviceMode].others.modelNumber) ? serviceElement[serviceElement.serviceMode].others.modelNumber : '-');
											appendActionDetailsString = appendActionDetailsString.replace('CATEGORY', checkForData(serviceElement[serviceElement.serviceMode].others.category) ? serviceElement[serviceElement.serviceMode].others.category : '-');
											appendActionDetailsString = appendActionDetailsString.replace('SERIALNUMBER', checkForData(serviceElement[serviceElement.serviceMode].others.serialNumber) ? serviceElement[serviceElement.serviceMode].others.serialNumber : '-');
											appendDetailsString = appendDetailsString + contextVar.blankString + contextVar.blankString + appendActionDetailsString;
										}
								...
								if (checkForData(serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes)) {
									if (arrayCheck(serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes.capturedPhoto)) {

										var servicePhotoCount = 0;
										serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes.capturedPhoto.forEach(function (photoElement) {
											servicePhotoCount++;
											var appendPhotoString = new String(contextVar.submittedEvidenceBody);
											appendPhotoString = appendPhotoString.replace('MATERIALPHOTO', checkForData(photoElement.servicePhotos.media) ? photoElement.servicePhotos.media : '-');
											appendPhotoString = appendPhotoString.replace('CAPTUREDON', formatDate(photoElement.servicePhotosCapturedTime));
											appendPhotoString = appendPhotoString.replace('ASSIGNEE', checkForData(photoElement.capturedBy) ? photoElement.capturedBy : '-');
											appendPhotoString = appendPhotoString.replace('TECHNICIANNOTES', checkForData(photoElement.servicePhotos.comment) ? photoElement.servicePhotos.comment : '-');

											contextVar.serviceDetailsPhotos = contextVar.serviceDetailsPhotos + appendPhotoString;

											if (servicePhotoCount % 3 == 0) {
												contextVar.serviceDetailsPhotos = contextVar.serviceDetailsPhotos + contextVar.footerForPhotos + contextVar.pageBreakFooter + contextVar.headerForPhotos;
											}
										});

									}

									contextVar.serviceDetailsPhotos = contextVar.headerForPhotos + contextVar.serviceDetailsPhotos;

									var appendComments = new String(contextVar.submittedEvidenceComments);
									appendComments = appendComments.replace('OVERALLCOMMENTS', checkForData(serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes.serviceOptionalNotes) ? serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes.serviceOptionalNotes : '-');

									contextVar.serviceDetailsPhotos = contextVar.serviceDetailsPhotos + appendComments + contextVar.footerForPhotos;

								}

								contextVar.serviceDetailsAllString = contextVar.serviceDetailsAllString + appendDetailsString + contextVar.pageBreakFooter + contextVar.submittedEvidenceHeader + contextVar.serviceDetailsPhotos + contextVar.footerString;
							...
							element.serviceGridWrapper.forEach(function (serviceElement) {
								if (checkForData(serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes) && arrayCheck(serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes.capturedPhoto)) {
									serviceElement[serviceElement.serviceMode].optionalPhotoPlusNotes.capturedPhoto.forEach(function (photoElement) {
										let photoName = contextVar.loopThroughTasksList_currentElement.taskId + "_serviceEvidence_" + formatDateString(photoElement.servicePhotosCapturedTime) + "_" + contextVar.serviceEvidenceKey;
										contextVar.fileNameArray.push(photoName);
										contextVar.photosArray.push(photoElement.servicePhotos.media);
										contextVar.serviceEvidenceKey++;
									});
								}
							});
						}
	...
	if (arrayCheck(contextVar.finalTaskResult.materialsDataGrid)) {
		if (contextVar.fileType == 'Closeout Package PDF') {
			contextVar.finalTaskResult.materialsDataGrid.forEach(function (materialElement) {
				...
				contextVar.materialsDetailsTableContents = contextVar.materialsDetailsTableContents + appendDetailsString + contextVar.blankSeparatorTableString + contextVar.submittedEvidenceHeader + contextVar.materialsDetailsPhotos + contextVar.footerString;
